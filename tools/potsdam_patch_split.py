...
def parse_args():
    ...
    parser.add_argument("--rgbnDSM-image", action='store_true')  # use Potsdam RGBnDSM format images
...
def get_img_mask_padded(image, mask, patch_size, mode):
    ...
    pad_mask = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right', border_mode=cv2.BORDER_CONSTANT,
                                value=0)(image=mask)          ##6 or 0

    img_pad, mask_pad = pad_img['image'], pad_mask['image']
    if args.rgbnDSM_image:
        img_pad = cv2.cvtColor(np.array(img_pad), cv2.COLOR_RGBA2BGRA)
    elif args.rgb_image:
...
def rgb_to_2D_label(_label):
    _label = _label[:,:,0:3].transpose(2, 0, 1)
    # print(_label.shape)
    ...
    label_seg[np.all(_label.transpose([1, 2, 0]) == Boundary, axis=-1)] = 6         #Boundary
    return label_seg
...
def patch_format(inp):
    (img_path, mask_path, imgs_output_dir, masks_output_dir, eroded, gt, rgb_image,rgbnDSM_image,
    ...
    if rgb_image:
        ...
        img_path = img_path + '_RGBnDSM.tif'
                ...
                if img_tile.shape[0] == split_size and img_tile.shape[1] == split_size \
                    ...
                    if cf[4] > 1.0 and mode == 'train':  # ignore car_aug, no improvement
                        ...
                        for i in range(len(car_imgs)):
                            ...
                            cv2.imwrite(out_img_path, car_imgs[i],((int(cv2.IMWRITE_TIFF_RESUNIT), 2,
                            ...
                            cv2.imwrite(out_mask_path, car_masks[i],[cv2.IMWRITE_PNG_COMPRESSION, 0])
                    else:
                        out_img_path = os.path.join(imgs_output_dir, "{}_{}_{}.tif".format(img_filename, m, k))
                        cv2.imwrite(out_img_path, img_tile,((int(cv2.IMWRITE_TIFF_RESUNIT), 2,
                                                             int(cv2.IMWRITE_TIFF_COMPRESSION), 1,
                                                             int(cv2.IMWRITE_TIFF_XDPI), 100,
                                                             int(cv2.IMWRITE_TIFF_YDPI), 100)))

                        out_mask_path = os.path.join(masks_output_dir, "{}_{}_{}.png".format(mask_filename, m, k))
                        cv2.imwrite(out_mask_path, mask_tile,     [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
if __name__ == "__main__":
    ...
    rgbnDSM_image = args.rgbnDSM_image
    mode = args.mode
    ...
    if rgbnDSM_image:
        img_paths = [p[:-12] for p in img_paths_raw]
    ...
    inp = [(img_path, mask_path, imgs_output_dir, masks_output_dir, eroded, gt, rgb_image,rgbnDSM_image,
